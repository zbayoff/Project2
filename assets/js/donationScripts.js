(function () {
    ...
    const btnAmounts = document.querySelector('.button-amounts');
    const btnAmountsBtns = document.querySelectorAll('.button-amounts button');

    const btnFreqs = document.querySelector('.button-freqs');
    const btnFreqsBtns = document.querySelectorAll('.button-freqs button');

    const otherAmount = document.querySelector('#other-amount');

    btnAmounts.addEventListener('click', selectBtnAmount);
    btnFreqs.addEventListener('click', selectFreqAmount);
    otherAmount.addEventListener('mouseup', clearBtnSelections);

    function selectBtnAmount(e) {
        clearBtnSelections();
    ...
    function clearBtnSelections() {
        for (let i = 0; i < btnAmountsBtns.length; i += 1) {
            btnAmountsBtns[i].classList.remove('selected');
        }
    }

    function selectFreqAmount(e) {
        clearFreqSelections();
        const selectedElement = e.target;
        selectedElement.classList.add('selected');
        e.stopPropagation();
    ...
    function clearFreqSelections() {
        for (let i = 0; i < btnFreqsBtns.length; i += 1) {
            btnFreqsBtns[i].classList.remove('selected');
        }
    }
